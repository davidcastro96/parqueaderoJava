...
public class Parqueadero {
     ...
     * @return Respuesta 1.
     *
     * deberá retornar la placa del carro que lleva más tiempo en el
     * parqueadero
     *
     */
    public String metodo1() {
        String placaCarroConMasTiempo = "";
        int tiempoParqueo = 0;
        Boolean sw = false;
        for (Puesto puesto : puestos) {
            if (puesto.estaOcupado()) {
                ...
                if (carro.darTiempoEnParqueadero(carro.darHoraLlegada()) > tiempoParqueo) {
                    placaCarroConMasTiempo = carro.darPlaca();
                    return placaCarroConMasTiempo;
                }
            }

        }
        if (!sw) {
            return "No hay vehiculos en el parqueadero";
        } else {
            return placaCarroConMasTiempo;
        }
    }
    ...
    public String metodo2() {
        int numeroPuestosTotales = TAMANO;
        int numeroPuestosOcupados = 0;
        for (Puesto puesto:puestos){
            if (puesto.estaOcupado()){
                numeroPuestosOcupados += 1;
            }
        }
        if (numeroPuestosOcupados >= numeroPuestosTotales){
            return "El parqueadero está lleno";
        }else{
            return String.valueOf("Puestos ocupados : "+numeroPuestosOcupados+"\nHay puestos disponibles");
        }
    }
}